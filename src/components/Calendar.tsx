...
import { ChevronLeft, ChevronRight, CalendarDays, Package, Flame, TrendingUp, TrendingDown, Minus } from 'lucide-react';
...
export default function Calendar({ events = [], onSelectDate }: CalendarProps) {
  ...
  // Trend vs the previous day, for the selected-day summary.
  const prevDay = new Date(selected.getFullYear(), selected.getMonth(), selected.getDate() - 1);
  const prevCount = counts[dayKey(prevDay)] ?? 0;
  ...
  return (
    <div className="bg-white shadow-md border border-gray-100 rounded-xl overflow-hidden">
      ...
      {/* Footer — selected-day summary */}
      <div className="px-3 py-3 bg-gray-50 border-t border-gray-100 flex items-center justify-between gap-2">
        <div className="flex items-center gap-2.5 min-w-0">
          {/* Date chip */}
          <div className="flex flex-col items-center justify-center w-10 h-10 rounded-xl bg-brand-blue text-white shrink-0 shadow-sm">
            <span className="text-[8px] font-semibold uppercase leading-none opacity-80">{WEEKDAYS[selected.getDay()]}</span>
            <span className="text-base font-bold leading-none mt-0.5">{selected.getDate()}</span>
          </div>
          {/* Count + date */}
          <div className="min-w-0">
            ...
            <p className="inline-flex items-center gap-1 text-[11px] text-gray-500">
              ...
              <span className="font-semibold text-gray-700">{selectedCount}</span> mizigo siku hii
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          <span
            title="Ikilinganishwa na siku iliyotangulia"
            className={`inline-flex items-center gap-0.5 text-[10px] font-bold px-1.5 py-1 rounded-lg ${
              delta > 0
